...
(async () => {
  ...
  try {
    await page.setContent('<!doctype html><html lang="en"><body><main id="advisor-root"></main></body></html>');
    ...
    assert.ok(await page.getByRole('button',{name:'Test connection / discover',exact:true}).isDisabled());
    await page.locator('#advisor-protocol').selectOption('ollama');
    await page.getByRole('button',{name:'Test connection / discover',exact:true}).click();
    ...
    assert.ok(await page.getByText('0.500 GB reported by the server',{exact:true}).isVisible());
    await page.getByRole('button',{name:'Delete from disk…',exact:true}).click();
    ...
    assert.ok(await page.getByRole('button',{name:'Confirm',exact:true}).isHidden());
    assert.equal(await page.getByRole('button',{name:'Delete from disk…',exact:true}).count(),0);
    await page.locator('#advisor-download-model').fill('tiny:latest');
    await page.getByRole('button',{name:'Download this model…',exact:true}).click();
    ...
    assert.ok(await page.getByRole('status').filter({hasText:'requires a structured response'}).isVisible());
    await page.locator('#advisor-mcp_response').selectOption('structured');
    await page.locator('#advisor-instructions').fill('Preserve the API.');
    await page.getByRole('button',{name:'Save configuration',exact:true}).click();
    last=await page.evaluate(()=>requests.at(-1)); assert.equal(last.config.instructions,'Preserve the API.'); assert.equal(last.investigates,true);
